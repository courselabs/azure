...
📋 Use Helm to install the chart from the `labs/kubernetes/helm/charts/whoami` folder, calling the release `whoami-default`.
...
Install the Nginx Ingress controller from the public Helm chart, using at least version `1.4.0` of the app. Use a new namespace called `ingress`. Browse to the HTTP endpoint and confirm you get a response from Nginx.
